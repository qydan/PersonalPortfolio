...
// px from top of viewport within which cursor triggers navbar to show
...
export default function Navbar() {
  ...
  // Show when cursor is near top, hide after delay when it moves away
  useEffect(() => {
    function handleMouseMove(e) {
      if (e.clientY < TRIGGER_ZONE || menuOpen) {
      ...
      {visible && (
        <motion.header
          ...
          className={`fixed top-0 left-0 w-full z-50 transition-colors duration-300 ${
            scrolled
              ? 'bg-white/30 dark:bg-gray-950/30 backdrop-blur-md shadow-sm'
              : 'bg-transparent'
          }`}
        >
          <nav
          ...
          >
                ...
                <li key={id}>
                  <button
                    ...
                    className={`text-sm font-medium transition-colors hover:text-red-500
                      ...
                      ${activeSection === id ? 'text-red-500' : 'text-gray-700 dark:text-gray-300'}`}
                  >
                    {label}
                  </button>
                </li>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                className="md:hidden p-2 rounded transition-colors hover:bg-gray-200 dark:hover:bg-gray-700
          ...
          <AnimatePresence>
            {menuOpen && (
              <motion.div
                ...
                className="md:hidden bg-white/90 dark:bg-gray-950/90 backdrop-blur-md shadow-md"
              >
                    ...
                    <li key={id}>
                      <button
                        ...
                        className={`w-full text-left px-6 py-3 text-sm font-medium transition-colors
                          hover:text-red-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-red-500
                          ${activeSection === id ? 'text-red-500' : 'text-gray-700 dark:text-gray-300'}`}
                      >
                        {label}
                      </button>
